...
/** On-heap row-major epoch-nano {@code long[]} chunk ({@link DateArrayChunk}). */
public final class HotDateArrayChunk implements DateArrayChunk {
    ...
    public HotDateArrayChunk(long[] values, int size, int elementsPerRow, Validity validity) {
    ...
    public static HotDateArrayChunk of(long[] rowMajorValues, int elementsPerRow) {
        ...
        return new HotDateArrayChunk(rowMajorValues, rows, elementsPerRow, Validity.allValid(rows));
    }
    ...
    @Override
    public long getEpochNano(int row, int element) {
        if (row < 0 || row >= size) {
