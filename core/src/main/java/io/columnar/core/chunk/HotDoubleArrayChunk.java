...
/** On-heap row-major {@code double[]} chunk ({@link DoubleArrayChunk}). */
public final class HotDoubleArrayChunk implements DoubleArrayChunk {
    ...
    public HotDoubleArrayChunk(
            double[] values, int size, int elementsPerRow, Validity validity) {
    ...
    public static HotDoubleArrayChunk of(double[] rowMajorValues, int elementsPerRow) {
        ...
        return new HotDoubleArrayChunk(
    ...
    @Override
    public double getDouble(int row, int element) {
        if (row < 0 || row >= size) {
